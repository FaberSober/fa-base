...
项目文件以 `fileId` 为主要标识，已有文件元数据、原文件地址、纯文本地址和 ONLYOFFICE 打开接口。因此需要增加一个统一的 `FilePreview` 门面组件，让业务页面只关心文件 ID、查看/编辑模式和展示容器。
...
| 模块 | 功能 | 功能详情 | 当前规划 | 进度 |
| --- | --- | --- | --- | --- |
| 文件预览基础 | `FilePreview` 统一入口 | 根据 `fileId`、文件元数据和 `mode` 自动选择查看器 | 执行开发 | ❌未完成 |
| 文件预览基础 | 文件资源解析 | 统一获取文件名、扩展名、MIME、大小和受控访问地址 | 执行开发 | ❌未完成 |
| 原生查看 | 图片查看 | 缩放、预览原图，沿用平台文件地址 | 执行开发 | ❌未完成 |
| 原生查看 | 音视频查看 | 使用浏览器原生媒体能力，处理加载失败 | 执行开发 | ❌未完成 |
| 原生查看 | PDF 查看 | 封装现有 PDF.js 能力，支持分页和基础工具栏 | 执行开发 | ❌未完成 |
| 原生查看 | 文本/代码查看 | 查看 TXT、JSON、XML、Markdown 和常见代码文件 | 执行开发 | ❌未完成 |
| 降级处理 | 通用文件回退 | FileViewer 或专用查看器失败时回退 kkFileView 或下载提示 | 执行开发 | ❌未完成 |
| Office 查看 | FileViewer 适配 | 对验证通过的 DOCX、XLSX、PPTX 提供只读查看 | 执行开发 | ❌未完成 |
| 展示容器 | 弹窗预览 | 提供 `FilePreviewModal`，封装触发器、拖拽、全屏和关闭 | 执行开发 | ❌未完成 |
| 展示容器 | 独立页面/Tab 预览 | 提供 `FilePreviewPage`，适配复杂文件和大尺寸查看器 | 执行开发 | ❌未完成 |
...
- 使用现有 `genLocalGetFile`、`genLocalGetFilePreview` 和 `getPlainFile` 生成受控地址。
