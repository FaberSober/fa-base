...
| 搜索选择 | 异步搜索治理 | 保留防抖搜索，只接受最新请求结果，补齐加载、空结果和失败状态 | 执行开发 | 🔍验证中 |
| 搜索选择 | 回写数据兼容 | 弹窗选择回传 ID 与带 `label` 的 option，兼容表格组合查询 | 执行开发 | 🕒待处理 |
| 弹窗选择 | 单选/多选语义 | 单选限制一人，多选支持跨部门、跨分页保留选择且去重 | 执行开发 | 🕒待处理 |
| 弹窗选择 | 取消与确认 | 打开时建立本地草稿，取消恢复原值，确认后统一触发 `onChange` | 执行开发 | 🕒待处理 |
| 弹窗选择 | 部门筛选安全 | 保留部门筛选，明确提供“全部用户”复位入口并关闭树拖拽 | 执行开发 | 🕒待处理 |
| 已选列表 | 用户详情缓存 | 批量补齐已选用户详情，避免每次增删重复请求并保持选择顺序 | 执行开发 | 🕒待处理 |
| 已选列表 | 身份信息展示 | 展示姓名、账号、部门、移除状态和空状态 | 执行开发 | 🕒待处理 |
