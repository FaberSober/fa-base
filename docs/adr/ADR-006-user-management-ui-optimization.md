...
- 状态：Proposed
- 日期：2026-09-19
- 范围：`frontend/apps/admin/features/fa-admin-pages` 用户管理页面、用户表单、用户详情、批量操作和用户导入导出
...
7. 首版不修改现有用户增删改、角色、部门和批量操作接口、数据库结构、公共 `BaseTree` 或 `FaFullContentModal` 的公共 API；用户导入导出允许增加用户专用接口和数据模型，不直接复用通用用户导入接口，页面级样式使用现有主题变量。
...
| 用户页面 | 新增入口优化 | 在右侧工具栏提供显式新增用户；保留部门树右键新增快捷入口 | 执行开发 | ✅已完成 |
| 用户查询 | 高频筛选 | 默认展示账户、姓名、手机号、工作状态，支持回车查询和重置 | 执行开发 | ✅已完成 |
| 用户查询 | 高级筛选与列配置 | 启用组合查询、查询场景和表格列配置，低频条件按需展开 | 执行开发 | ✅已完成 |
| 用户列表 | 核心字段重排 | 聚合人员身份信息，突出部门、角色、工作状态、账户状态和后台访问 | 执行开发 | ✅已完成 |
| 用户列表 | 状态操作反馈 | 状态更新显示加载、成功和失败结果，失败时恢复列表状态 | 执行开发 | ✅已完成 |
| 批量操作 | 批量部门/角色/密码 | 保留现有接口，优化已选人数提示、按钮层级、确认和完成后的刷新 | 执行开发 | ✅已完成 |
| 用户编辑 | 全屏表单容器 | 新增和编辑统一改用 `FaFullContentModal`，跟随当前 Tab 面板并支持主体滚动 | 执行开发 | ✅已完成 |
| 用户编辑 | 分组表单与状态管理 | 表单分为基础资料、账号权限、补充资料；打开时清理旧值，异步角色回填完成后才允许提交 | 执行开发 | ✅已完成 |
| 用户详情 | 安全信息收敛 | 移除密码展示，API Token 默认脱敏或不展示，按业务权限决定是否提供单独操作 | 执行开发 | ✅已完成 |
| 用户导出 | 用户专用导出契约 | 使用独立导出模型，导出字段与导入字段解耦；严格遵守当前租户、部门范围和超级用户权限，不导出密码、Token 等敏感或内部字段 | 执行开发 | ✅已完成 |
| 用户导入 | 用户专用导入预览与提交 | 使用独立导入模型，支持模板下载、部门与角色解析、行级错误反馈、新增/更新统计；全部校验通过后事务性提交，不直接使用通用 `importExcel` | 执行开发 | ✅已完成 |
| 主题适配 | 主题、响应式和可访问性 | 使用 `--fa-*` 变量，验证亮色/暗色、窄屏、键盘焦点和表单错误状态 | 执行开发 | ✂️剪枝放弃本条目 |
| 工程质量 | 页面级验证 | 完成受影响文件的格式检查、局部类型检查和用户管理关键流程回归 | 执行开发 | ✅已完成 |
...
- 导入导出使用独立的 Excel 数据模型，不直接把持久化实体 `User` 作为完整 Excel 契约。
- 导出复用用户列表的查询条件和服务端数据范围：普通模式遵守当前租户及部门子树范围，超级用户模式执行明确的跨租户权限校验。
- 导出不包含密码、API Token、超级管理员标记、最后在线时间和内部文件标识等敏感或运行时字段；导入模板中的初始密码仅用于新增用户，不能出现在导出文件中。
- 导入使用用户 ID 作为更新匹配键，没有用户 ID 的行视为新增；部门使用完整部门路径解析，角色使用逗号分隔的角色名称解析，无法唯一匹配时按行返回错误。
- 导入先完成整批校验，再执行事务性提交；校验失败时不落库，并返回可定位到行的错误信息。新增用户密码必须经过现有用户业务逻辑加密，角色和租户关系必须通过现有业务逻辑维护。
- 页面复用现有文件上传、模板下载和列表刷新能力，但不直接将通用 `CommonExcelUploadModal` 连接到用户的通用 `importExcel` 接口。
...
- 超级管理员账户状态固定为有效；列表开关、编辑表单和后端更新接口均不得将其设置为无效。
...
- 不修改现有用户增删改、角色、部门和批量操作后端接口及数据库结构；导入导出仅增加用户专用接口和数据模型，不修改通用导入导出基类。
...
- 导出结果与当前筛选条件、租户和部门范围一致，不包含密码、Token 等敏感字段；超级用户跨租户导出需要服务端权限校验。
- 可下载用户导入模板；导入预览能展示新增、更新和错误数量，错误信息能定位到具体行；存在错误时不产生部分落库。
- 导入成功后新增用户密码已加密，部门、角色和租户关系正确维护；更新用户时不覆盖原有密码，用户 ID 匹配和权限范围校验有效。
...
- 状态开关失败时列表不会保留错误状态，超级管理员账户始终有效，后台访问限制不被绕过。
...
5. 设计用户专用导入导出模型、范围校验、模板和导入预览/提交流程。
6. 补齐主题、响应式、可访问性、空状态和异常反馈。
7. 执行局部格式检查、类型检查和用户管理关键流程回归。
...
- `frontend/apps/admin/features/fa-admin-pages/components/excel/CommonExcelUploadModal.tsx`
...
- `fa-base/src/main/java/com/faber/api/base/admin/vo/ret/UserExportVo.java`
- `fa-core/src/main/java/com/faber/core/web/biz/BaseBiz.java`
