...
| 3 | `fa-base` 远程日志 | 采集控制与日志路由 | 管理员按权限启停指定客户端，日志只发给发起会话的管理员 | 执行开发 | ✅已完成 |
...
3. **采集控制与日志路由**：管理员通过现有 WebSocket 消息启停目标客户端日志，服务端逐次校验权限并记录操作。日志仅转发给发起采集的管理员；管理员离开、客户端断开或采集超时后停止会话。
