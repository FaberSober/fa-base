...
| 7.1 | `fa-base` 在线状态 | 重复连接去重与单连接断开 | 同一设备多个 WebSocket 连接只计一台；关闭其中一个连接后，其余活跃连接仍保持设备在线 | 本期实施 | ✅已完成 |
| 7.2 | `fa-base` 在线状态 | 60 秒心跳过期 | 超过 60 秒未收到心跳的连接不再显示为在线设备 | 本期实施 | ✅已完成 |
| 7.3 | `fa-base` 在线状态 | 旧客户端兼容标识 | 未上报 `clientInstanceId` 时使用 WebSocket 连接 ID 作为临时设备标识，并保留 60 秒 TTL | 本期实施 | ✅已完成 |
| 7.4 | `fa-base` 在线用户 API | 多客户端汇总 | 同一用户的 Web、APP、桌面端设备聚合为一行，平台数量和设备总数准确 | 本期实施 | ✅已完成 |
| 7.5 | `fa-base` 在线用户 API | 在线设备权限校验 | 无平台查看权限的用户不能查询在线用户汇总或设备明细，拒绝时不读取在线状态 | 本期实施 | ✅已完成 |
| 7.6 | `fa-base` 在线状态 | 跨节点共享在线状态 | 一个节点写入的在线连接可由另一节点读取并移除 | 本期实施 | ✅已完成 |
