...
| 2 | `fa-base` | 定向测试发送与状态 API | 向所选设备发送通知；按设备返回服务端受理或失败结果，并关联测试编号 | 执行开发 | 🕒待处理 |
| 3 | `mobile/fa-core-mobile` | 接收与点击回执 | 将测试通知的接收、点击事件回传服务端；校验后处理应用内跳转 | 执行开发 | 🕒待处理 |
| 4 | `fa-admin-demo-pages` | 目标设备选择 | 搜索并勾选已注册设备，默认单设备，单次最多发送 5 台 | 执行开发 | 🕒待处理 |
| 5 | `fa-admin-demo-pages` | 测试消息编辑与预览 | 编辑标题、内容、应用内链接和可选扩展 JSON，发送前预览确认 | 执行开发 | 🕒待处理 |
...
2. **测试发送**：提供仅供管理员测试使用的接口，校验设备已启用、未失效且属于允许的 App/环境。复用 `ADR-mobile-006` 规划的 UniPush 服务端发送能力；按设备返回 Provider 受理或失败结果，并在推送数据中携带不可预测的 `testId`。
