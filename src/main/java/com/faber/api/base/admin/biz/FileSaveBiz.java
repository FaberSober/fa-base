...
public class FileSaveBiz extends BaseBiz<FileSaveMapper, FileSave> {
    ...
    public FileSave upload(MultipartFile file) throws IOException {
        ...
        fileSaveEntity.setSize(file.getSize() / 1000);
        fileSaveEntity.setUrl(url);

        // local img preview
        fileSaveEntity.setImgPreview(fileHelper.getImgPreview(url));

        save(fileSaveEntity);

        fileSaveEntity.setLocalUrl("/api/base/admin/fileSave/getFile/" + fileSaveEntity.getId());
    ...
    public void getFilePreview(String fileId) throws IOException {
        ...
        switch (fileSaveEntity.getDrive()) {
            case LOCAL:
                FileHelperLocal.getLocalFilePath(fileSaveEntity.getImgPreview());
                break;
            case QINIU:
            case ALI:
            case TX:
                // TODO 这里可以针对返回不同存储的预览图片URL
                HttpServletResponse response = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getResponse();
                response.sendRedirect(URLUtil.encode(fileSaveEntity.getUrl()));
                break;
        }
