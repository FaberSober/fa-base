...
SELECT 12040500, 12040000, 1, '在线用户', 4, 1, 'mdi:account-network-outline', 1, 1,
       '/admin/system/monitor/onlineUser', CURRENT_TIMESTAMP, '1', '超级管理员', '127.0.0.1', 0
WHERE NOT EXISTS (SELECT 1 FROM `base_rbac_menu` WHERE `id` = 12040500);

INSERT INTO `base_rbac_menu` (`id`, `parent_id`, `scope`, `name`, `sort`, `level`, `icon`, `status`, `link_type`, `link_url`, `crt_time`, `crt_user`, `crt_name`, `crt_host`, `deleted`)
SELECT 12040501, 12040500, 1, '强制下线', 0, 9, NULL, 1, 1,
       '/admin/system/monitor/onlineUser:kickout', CURRENT_TIMESTAMP, '1', '超级管理员', '127.0.0.1', 0
...
SELECT 1, m.`id`, 0, CURRENT_TIMESTAMP, '1', '超级管理员', '127.0.0.1', 0
FROM `base_rbac_menu` m
WHERE m.`id` IN (12000000, 12040000, 12040500, 12040501)
  -- 空库由 initAdminRoleMenu 统一初始化全部权限，避免提前插入导致其跳过。
  AND EXISTS (SELECT 1 FROM `base_rbac_role_menu` existing WHERE existing.`role_id` = 1 AND existing.`deleted` = 0)
  AND NOT EXISTS (
