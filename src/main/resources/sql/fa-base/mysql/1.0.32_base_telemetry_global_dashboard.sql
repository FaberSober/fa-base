...
SELECT 10030006, 10030000, 1, '全局概览', 0, 1, NULL, 1, 1,
       '/admin/system/telemetry/overview', CURRENT_TIMESTAMP, '1', '超级管理员', '127.0.0.1', 0
WHERE NOT EXISTS (
...
SELECT 1, 10030006, 0, CURRENT_TIMESTAMP, '1', '超级管理员', '127.0.0.1', 0
WHERE EXISTS (
    SELECT 1 FROM `base_rbac_role_menu` WHERE `role_id` = 1 AND `deleted` = 0
  )
  AND NOT EXISTS (
    SELECT 1 FROM `base_rbac_role_menu` WHERE `role_id` = 1 AND `menu_id` = 10030006 AND `deleted` = 0
  );
